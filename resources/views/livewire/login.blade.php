<form method="post" action="/foobar" class="pt-1/3 pt-18 m-4 flex flex-col gap-4 p-6">
  ...
  <div class="pt-10 text-center text-sm">
    <a href="/registration" wire:navigate>{{ __('Signup') }}</a>
    <span>/</span>
    <a href="/send-password-change-email" wire:navigate>{{ __('Reset password') }}</a>
    <span>/</span>
    <a href="/registration-complete" wire:navigate>{{ __('Activate account') }}</a>
  </div>
